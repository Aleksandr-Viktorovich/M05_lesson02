import {overlay, totalModal, totalTable} from "./elementsDOMmodule.js";
import {setStorage, removeStorage} from './storageModule.js';

export const createRow = (id, title, category, units, count, price, totalPrice) => {
  const row = document.createElement('tr');
  row.classList.add('table__row');
  row.innerHTML = `
    <tr class="table__row">
        <td class="goods__id" id="">${id}</td>
        <td>${title}</td>
        <td>${category}</td>
        <td>${units}</td>
...
export const addGoodsPage = (goods, table) => {
  let goodsId = Math.floor(Math.random() * 1000000000);
  goods.id = goodsId;
...
export const formControl = (modal, table) => {
